#!/usr/bin/env python3
"""Cross-encoder reranker evaluation — three-way NDCG@10 comparison.

Evaluates one or more cross-encoder rerankers against the LLM-judge cache used
as graded ground truth, on an apples-to-apples set of (query, doc) candidates.
...
4. For each model: score every (query, doc_text) pair with CrossEncoder.predict
   (batched, on CUDA when available), rank each query's candidates by score,
   compute NDCG@10 (gain = 2^grade - 1, ideal-DCG from the grade-sorted order)
   and MRR@10 (first grade>=2). Average over qualifying queries.
...
─────
    ...
    python scripts/eval_cross_encoder.py \
        ...
        --models base=cross-encoder/ms-marco-MiniLM-L-6-v2 \
                 baseline=models/sfu-cross-encoder-baseline \
                 v1=models/sfu-cross-encoder-v1 \
        --output data/eval_results/cross_encoder_eval.json
"""
import argparse
import json
import logging
...
DEFAULT_OUTPUT = REPO_ROOT / "data/eval_results/cross_encoder_eval.json"
...
def evaluate_model(
    ...
    """Score the shared eval set with one cross-encoder, return aggregate metrics."""
    from sentence_transformers import CrossEncoder
    import torch

    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger.info("[%s] loading %s on %s", label, model_path, device)
    ce = CrossEncoder(model_path, device=device)

    # Flatten all (query, text) pairs into one big batch for efficiency, then
    # slice scores back per query.
    pairs: list[tuple[str, str]] = []
    spans: list[tuple[str, int, int]] = []  # (query, start, end)
    for query, rows in eval_set.items():
        start = len(pairs)
        for _doc_id, text, _grade in rows:
            pairs.append((query, text))
        spans.append((query, start, len(pairs)))

    logger.info("[%s] scoring %d pairs across %d queries...", label, len(pairs), len(eval_set))
    scores = ce.predict(
        pairs, batch_size=PREDICT_BATCH, show_progress_bar=False
    ).tolist()
    ...
    for query, start, end in spans:
        rows = eval_set[query]
        qscores = scores[start:end]
        # Rank candidates by descending model score; grades follow.
    ...
    return {
        ...
        "queries": n,
        "candidates_scored": len(pairs),
    }


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cross-encoder reranker NDCG@10 evaluation vs LLM-judge ground truth",
    ...
    # Determine winner + deltas vs base (the first 'base' label, else first model).
    base_result = next((r for r in results if r["label"] == "base"), results[0])
    ...
    # ── Print table ──
    print("\n" + "=" * 78)
    print(f"CROSS-ENCODER RERANKER EVALUATION  (NDCG@{K} vs LLM-judge ground truth)")
    print("=" * 78)
